# Speakeraggio — Val Stàffora / Passo del Giovà

Copione per registrazione voce.
Ogni stacco di riga e' una pausa: non riempirla, sotto respirano motore e musica.

Pronuncia: **Stàffora** accento sulla prima "a" · **Giovà** accento sulla "a"
finale · **Lèsima** accento sulla "e".

---

Tutti salgono al Penice. Tutti.

E intanto, due valli più in là, questa strada è vuota.


Val Stàffora. Sul telefono la rotta è già tutta lì: risaliamo la valle fino in
fondo, poi su al Passo del Giovà.


Alle spalle, Varzi. I portici, la torre, e per secoli una dogana: di qui passava
la Via del Sale.


Genova era di là. Il sale saliva a dorso di mulo, fin quassù.


La strada di oggi ricalca quelle mulattiere. Per questo non va mai dritta: segue
l'acqua finché può, poi si arrampica.


Non è una strada da record. Asfalto stretto, curve che si chiudono, niente
tornanti da cartolina.

Però è tua. Di domenica, qui, incroci tre macchine. Al Penice sei in fila dietro
un camper.


Il bosco si chiude sopra la testa. L'aria cala di colpo. Sa di terra bagnata.


Un borgo di pietra. Quattro case, un muro a secco, le persiane chiuse. Quassù ci
vive qualche decina di persone. D'inverno, meno.

Si passa piano. Quasi per rispetto.


Si sale ancora. Roccia tagliata, guardrail che sparisce. E quando il verde si
apre, sul crinale c'è la palla bianca del Lesima.


Il Giovà è questo.

Niente bar. Niente adesivi. Niente file di moto parcheggiate.

Solo il punto dove la Lombardia finisce e comincia l'Appennino.


Novanta minuti da Milano. Il tempo di andare all'IKEA e tornare.

E sembra di stare a quattrocento chilometri da tutto.


[NOME DEL CANALE]. Ci vediamo alla prossima curva.

---

## Titolo a schermo, primi quattro secondi

    IL PASSO CHE MILANO NON SA DI AVERE
    Val Stàffora -> Giovà, Oltrepò Pavese

## Dati da verificare prima di registrare

- "due valli più in là" e la strada vuota: regge come descrizione del tratto?
- la palla del Lesima e' davvero visibile da quel punto del crinale
- quota del passo, se vuoi citarla
- nome del ponte coperto, se vuoi citarlo

## Note tecniche sul file video

- watermark `clideo.com` in basso a destra per tutta la durata: rifare
  l'export dal sorgente
- dal secondo 124 compare la barra "Interrompi registrazione"
- formato 784x1248: esportare a 1080x1920 per Reel e TikTok
- nessuna traccia audio: serve sonoro motore o base musicale sotto la voce
- musica: tappeto atmosferico sotto tutta la narrazione, innesto piu' ampio
  negli ultimi venti secondi, da "Novanta minuti da Milano"

---

## Appendice per il montaggio

Non serve in fase di lettura. I blocchi, nell'ordine, si appoggiano a questi
punti del video:

    0:00  hook, valle aperta
    0:09  LA ROTTA — mappa in quadro, attraversamento del ponte
    0:21  Varzi e la Via del Sale
    0:33  Genova e le mulattiere (mappa ancora leggibile)
    0:45  la strada ricalca i tracciati
    0:59  "non è una strada da record"
    1:14  il bosco
    1:29  il borgo
    1:43  roccia tagliata e Lesima
    1:56  il Giovà
    2:14  chiusura
    2:24  firma

Finestre in cui il telefono con la mappa e' leggibile: 0:12-0:14 (la migliore),
0:33-0:38, 0:58-1:02.
